...
@Component({
  selector: 'page-patient-nihss-calculator',
  templateUrl: 'patient-nihss-calculator.html',
})
export class PatientNihssCalculatorPage {
  ...
  saveNIHSSScore(){
    ...
    for(let i=0; i <= this.nihss_default_values.length - 1; i++){
      ...
      var submitNIHSSData = {
        ...
        'nihss_time' : this.navParams.get('nihssTime'),
        'nihss_value' : this.total_calculated_points,
        'nihss_options' : JSON.stringify(this.selectedNihssData),
      }
      this.webApi.updateNIHSSofPatient(this.usertokenData, submitNIHSSData).then((result) => {
        if (result['data'] && result['data']) {
          this.selectedNihssData = result['data']['nihss_data']['nihss_options'];
